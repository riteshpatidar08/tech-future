...
interface CourseCardProps {
  ...
  gradient: string;
  courseId: string;
  onEnroll?: (courseTitle: string) => void; // Optional callback to open enrollment form
}
  ...
  const getLogoColors = (id: string) => {
    switch (id) {
      ...
      case 'machine-learning':
        return {
          bg: 'rgba(127,109,255,0.08)',
          border: 'rgba(127,109,255,0.2)',
          icon: '#7F6DFF',
        };
      case 'data-analytics':
        return {
  ...
  return (
    <div className="group relative bg-white rounded-lg border border-slate-200 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden">
      ...
      <div className="relative p-4 z-10">
        ...
        <div className="mb-3">
          <h4 className="font-semibold text-slate-900 mb-1.5 text-xs">
            What you'll learn:
          </h4>
          <ul className="space-y-1">
            {features.map((feature, index) => (
              <li
                key={index}
                className="text-xs text-slate-600 flex items-center bg-slate-50 p-1.5 rounded-md"
              >
        ...
        <div className="space-y-3">
          ...
          {onEnroll ? (
            <Button
              ...
              className="w-full border-2 border-slate-200 text-slate-700 hover:bg-slate-50 transition-all duration-300 font-semibold py-3 rounded-lg"
            >
              Enroll Now
            </Button>
          ) : (
